...
%ShapeIndex3_HK = 2/pi*atan(mean_curv3./(sqrt(mean_curv3.^2-Gaussian_curv3)));
%%%%%%%%%%%%%%%%%%%
figure(10)
binWidth = 0.01;
...
print('Compare Three shape index_human_left_scatter.tif','-dtiff')
...
set(0,'defaultaxesfontsize',22);
print('Compare Three meancurv_human_left_scatter.tif','-dtiff')
...
% figure(13)
% h3_shape_HK = histogram(ShapeIndex3_HK)
% h3_shape_HK.Normalization = 'probability';
% h3_shape_HK.BinWidth = 0.01;
% title('Shape Index distribution (HK)')
% 
...
% 
% figure(1)
% h3 = histogram(ShapeIndex3);hold on;
% h3.FaceColor = [0 0 0];
% h3.Normalization = 'probability';
% h3.BinWidth = 0.01;
% % title('histogram of shape index')
% % print('histogram of shape index.tif','-dtiff')
% 
% h1 = histogram(ShapeIndex1);
% h1.FaceColor = [0 0 1];
% 
% h2 = histogram(ShapeIndex2)
% h2.FaceColor = [ 0.5381    0.3505    0.3505];
% 
...
% 
% 
% legend('real','simulation','gel','Location','northwest');
% print('Compare Three shape index.tif','-dtiff')
% 
% figure(2)
% set(gcf,'position',[50,50,1000,800])
% %plot_meshg_lm(v3,f3,lm3,ShapeIndex3);
% plot_meshj(v3,f3,ShapeIndex3);
% %title([lr{i},' real ferret brain'])
% view([0 10])  
% colorbar
% print([name,'_curv_real_lm_SI_jet.tif'],'-dtiff');
...
% set(gcf,'position',[50,50,1000,800])
% %plot_meshg_lm(v3,f3,lm3,ShapeIndex3);
% plot_meshg(v3,f3,ShapeIndex3);
% %title([lr{i},' real ferret brain'])
% view([0 10])  
% colorbar
% print([name,'_curv_real_lm_SI_gray.tif'],'-dtiff');
% 
...
figure(3) 
set(gcf,'position',[50,50,1000,800])
plot_meshp_lm(v2,f2,lm2,threshold(mean_curv2,0.75));
%title([lr{i},' real ferret brain'])
view([0 20])  
...
figure(6)
set(gcf,'position',[50,50,1000,800])
plot_meshp_lm_disk(map23,f2_filled,lm2,threshold(mean_curv2,0.75));
%title([lr{i},' simulated ferret brain'])
view([90 90])  
...
fid = fopen('Similarity_ShapeIndexThree_human0426.txt','a+')
fprintf(fid,name)
fprintf(fid,'\r\n')
fprintf(fid,'\t')
...
% 2/4/2024 modified--p-norm
score13_p1 = 1 - 0.5*sum(abs(I1-I3))/length(I1)
% score13_p2my = 1 - 0.5*sqrt( sum( abs(I1-I3).^2 ))/length(I1)
 score13_p2 = 1 - 0.5*sqrt( sum(abs(I1-I3).^2)/length(I1) )
 score13_p3 = 1 - 0.5*( sum(abs(I1-I3).^3)/length(I1)).^(1/3)
% 
...
score23_pinf = 1 - max(abs(I2-I3)/length(I2))



fid = fopen('Similarity_rescaled_meancurv_human_Three_0331.txt','a+')
fprintf(fid,name)
fprintf(fid,'\r\n')
fprintf(fid,'\t')
fprintf(fid,'simu_p1=%8.4f, gel_p1=%8.4f\r\n',score13_p1,score23_p1)
...
% 
% % similarity score
% score23_p1 = 1 - sum(abs(I2-I3))/length(I2)
% score23_p2my = 1 - sqrt( sum( abs(I2-I3).^2 ))/length(I2)
% score23_p2 = 1 - sqrt(sum(abs(I2-I3).^2)/length(I2))
% 
% score23_pinf = 1 - max(abs(I2-I3)/length(I2))
% 
% 
% 
% fid = fopen('Similarity_human.txt','a+')
% fprintf(fid,name)
% fprintf(fid,'\r\n')
% fprintf(fid,'\t')
% fprintf(fid,'simu_p1=%8.4f, gel_p1=%8.4f\r\n',score13_p1,score23_p1)
