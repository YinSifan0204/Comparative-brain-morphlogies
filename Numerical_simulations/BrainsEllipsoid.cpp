...
//	int SN[nsn];
	...
	const double alfagt = 1.9;// Tangential growth rate of gray matter, related to R/T ratio
...
//	wb = 1;
	...
	ofstream dat;
	dat.precision(5);
	dat.open("Brains_v1_edited_by_Gary_log.dat");
	...
	while (t < 1.0) { // Main loop
		...
		// Folding simulation
		H = 0.1 - 0.05*t;
		// should contain at least eight layers, i.e. > 8*a?
//		zoom = 2.2 - 1.2*t;
//		alfagt = 1+(2.83-1)*t;
		...
		for (int i = 0; i < ne; i++) {
			...
			Matrix Lg = Lgg*gm + Lgw*vm; //  = Matrix(...)*alfagt*gm
...
void createNNLtriangle(vector<int>* NNLt, Vector* Ut, vector<Face>& faces, int* SN, int nsn, int nf, double hs, double mw) {
	// mw: 8.0*a, where a is the spacing	// Width of a cell in the linked cell algorithm for proximity detection
	int mx = max(1, (int)(5.0/mw)); // usually >> 1
	vector<int> head(mx*mx*mx, -1); // like a 3D Box
	vector<int> list(nf);
	int xa, ya, za, xi, yi, zi;
//	double ub, vb, wb;
	int pt, tri;
	Vector cog; // center of each triangle
	...
	for (int i = 0; i < nf; i++) { // Divide triangles into cells
		...
		int xa = (int)((cog.x + 2.5)/5.0*mx); // usually < mx
		int ya = (int)((cog.y + 2.5)/5.0*mx); // usually < mx
		int za = (int)((cog.z + 2.5)/5.0*mx); // usually < mx
	...
	for (int i = 0; i < nsn; i++) { // Search cells around each point and build proximity list
		int pt = SN[i]; // surface node
		NNLt[i].clear(); // clear
		int xa = (int)((Ut[pt].x + 2.5)/5.0*mx); // usually < mx
